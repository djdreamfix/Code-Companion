...
export function MarkPopup({ mark }: MarkPopupProps) {
  ...
  const colorLabel = 
    mark.color === "blue" ? "Chill Spot" :
    mark.color === "green" ? "Active Zone" : "Wild Event";
    
  const colorClass = 
    mark.color === "blue" ? "text-blue-600 bg-blue-50" :
    mark.color === "green" ? "text-green-600 bg-green-50" : "text-purple-600 bg-purple-50";

  return (
    <div className="w-64 p-4 font-sans">
      <div className="flex items-center justify-between mb-3">
        <span className={`text-xs font-bold uppercase tracking-wider px-2 py-1 rounded-md ${colorClass}`}>
          {colorLabel}
        </span>
      </div>
